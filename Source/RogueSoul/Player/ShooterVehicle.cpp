	...
	if (LocalCharacter)
	{
		...
		if (!LocalSitSocket.IsEqual(FName(TEXT("None"))))
		{
			...
			if (IsDriver)
			{
				...
				{
					if (GetRemoteRole() == ROLE_Authority)LocalCharacter->GetController()->Possess(this);
	...
	if (LocalCharacter)
	{
		...
		if (AllPassengers.Remove(LocalCharacter))
		{
			...
			if (DriverCharacter && LocalCharacter->GetName() == DriverCharacter->GetName())
			{
				...
				{
					if (GetRemoteRole() == ROLE_Authority) this->GetController()->Possess(LocalCharacter);
...
bool AShooterVehicle::CanDie() const
{
	if (bIsDying										// already dying
		...
		|| GetRemoteRole() != ROLE_Authority)						// not authority
	{
		return false;
	}
...
void AShooterVehicle::OnDriverDeath(AShooterCharacter* Character)
{
	...
	{
		if (this->GetController() && GetRemoteRole() == ROLE_Authority)
		{
...
void AShooterVehicle::OnChangePlayerType(EShooterPlayerType ePlayerType)
{
	...
	case EShooterPlayerType::TSS:
		...
		SpringArm->SetRelativeRotation(GetClass()->GetDefaultObject<AShooterVehicle>()->SpringArm->GetRelativeRotation() + TSS_SprintArmRotationOffset);
