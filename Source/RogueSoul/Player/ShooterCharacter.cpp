...
{
	if (GetRemoteRole() == ROLE_Authority && !bIsDying)
	{
...
{
	if (Weapon && GetRemoteRole() == ROLE_Authority)
	{
...
void AShooterCharacter::OnChangePlayerType(EShooterPlayerType ePlayerType)
{
	...
	if (SpringArm)
	{
		...
		case EShooterPlayerType::TSS:
			...
			SpringArm->SetRelativeRotation(GetClass()->GetDefaultObject<AShooterCharacter>()->SpringArm->GetRelativeRotation() + TSS_SprintArmRotationOffset);
